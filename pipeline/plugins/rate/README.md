...
## Docker
The rate plugin can be easily build and run using [Docker](https://www.docker.com/). 

### Running the pre-built docker image
A pre-built image is available at the [docker hub repository](https://hub.docker.com/r/sergdelft/hyperion).
The tag to use is `sergdelft/hyperion:pipeline-plugins-rate-0.1.0`, for the latest version please check the repository.
...
The included Dockerfile compiles the rate plugin into a fat jar and copies it to a new image which runs the plugin with the given config.
...
after building is complete you can run the rate.
