...
A pre-built image is available at dockerhub under `daveter9/hyperion-pipeline-plugins-extractor:0.1.0`.
To run this image with `extractor_config.yml` as its configuration execute:

```shell script
docker run -it -rm -v ${PWD}/extractor_config.yml:/root/config.yml daveter9/hyperion-pipeline-plugins-extractor:0.1.0
```
